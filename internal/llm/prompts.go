...
id (格式为 "scene_<序号>"), title (场景标题), sequence (整数), location (地点), location_type (地点类型，如"门派""洞府""城镇""荒野""秘境"), time_of_day (时间，如"清晨""黄昏""夜晚"), atmosphere (氛围描述), summary (场景概要), chapter_source (整数，对应原文章节号), characters_present (数组，每个元素包含 id 和 state), mood (情绪基调)
